...
import 'pro_network_intelligence_screen.dart';
...
class NoBarsProScreen extends StatelessWidget {
        ...
        builder: (context, connector, settingsService, _) {
          ...
          return ListView(
            ...
            children: [
              ...
              const SizedBox(height: 18),
              _section('PRO NETWORK INTELLIGENCE'),
              _card([
                _navTile(context,
                  icon: Icons.analytics_outlined,
                  color: _yellow,
                  title: 'Network Health & Operator Center',
                  subtitle: 'Route scores, watchlist, anomalies, session recorder, coverage, emergency view and reports',
                  screen: const ProNetworkIntelligenceScreen()),
                _divider(),
                _navTile(context,
              ...
              _card([
                _statusTile(
                  ...
                  subtitle: connector.isAutoReconnectScheduled ? 'Reconnect attempt scheduled now' : 'Automatic recovery armed for unexpected BLE drops'),
                _divider(),
                _statusTile(icon: Icons.battery_charging_full, color: _blue, title: 'Node battery', subtitle: _batteryText(connector)),
                _divider(),
                _navTile(context,
                  icon: Icons.bluetooth_searching,
                  color: _yellow,
                  title: connector.isConnected ? 'Switch device' : 'Connect a device',
                  subtitle: connector.isConnected ? 'Currently connected to ${connector.deviceDisplayName}' : 'Scan for a MeshCore companion over Bluetooth',
              ...
              _card([
                ...
                const ListTile(
                  ...
                  subtitle: Text('Resolver foundation retained for trusted public-directory path-prefix lookups'),
                  trailing: _BetaChip()),
              ]),
              const SizedBox(height: 18),
              const Text(
  ...
  Widget _hero(BuildContext context, MeshCoreConnector connector) => Container(
    padding: const EdgeInsets.all(20),
    decoration: BoxDecoration(
      borderRadius: BorderRadius.circular(22),
      border: Border.all(color: _pink.withValues(alpha: .6)),
      gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [_pink.withValues(alpha: .16), _blue.withValues(alpha: .10), _yellow.withValues(alpha: .05)])),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text(connector.isConnected ? 'Live mesh intelligence is active on ${connector.deviceDisplayName}.' : 'Connect a node to activate live traffic, route intelligence and network diagnostics.', style: Theme.of(context).textTheme.bodyMedium),
    ]));

  Widget _statsCard(MeshCoreConnector connector) => _card([
  ...
  Widget _section(String text) => Padding(padding: const EdgeInsets.fromLTRB(6, 0, 6, 7), child: Text(text, style: const TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.w700, letterSpacing: 1.15)));
  ...
  Widget _statusTile({required IconData icon, required Color color, required String title, required String subtitle}) => ListTile(leading: Icon(icon, color: color), title: Text(title), subtitle: Text(subtitle));
  Widget _navTile(BuildContext context, {required IconData icon, required Color color, required String title, required String subtitle, required Widget screen}) => ListTile(leading: Icon(icon, color: color), title: Text(title), subtitle: Text(subtitle), trailing: const Icon(Icons.chevron_right), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => screen)));
...
class _BetaChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(color: const Color(0xFFFF1493).withValues(alpha: .12), borderRadius: BorderRadius.circular(99), border: Border.all(color: const Color(0xFFFF1493).withValues(alpha: .45))),
