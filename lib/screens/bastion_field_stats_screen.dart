...
class BastionFieldStatsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final recent = contacts
        .where((c) => now.difference(c.lastSeen).inMinutes <= 60)
        .length;
    final routed = contacts
        .where((c) => c.pathLength > 0 || c.path.isNotEmpty)
        .length;
      ...
      body: ListView(
        ...
        children: [
          Container(
            ...
            child: const Column(
              ...
              children: [
                ...
                SizedBox(height: 6),
                Text(
                  'These counters are calculated from the connected MeshCore contact table, locations, paths and last-seen timestamps.',
                  style: TextStyle(color: Color(0xFFB8C1C9), height: 1.4),
                ),
              ],
            ),
          ),
          ...
          const _Panel(
            Icons.route_rounded,
            'PATH INTELLIGENCE',
            'Bastion’s path engine separately tracks attempts, hop counts, success/failure, trip time and route weighting. The Path Analysis module is being expanded around those persisted records.',
          ),
          const SizedBox(height: 10),
          const _Panel(
            Icons.map_rounded,
            'COVERAGE SESSIONS',
            'Coverage Capture is active as a field-session module. Radio event + GPS persistence is the next wiring layer before we call it true wardrive logging.',
          ),
        ],
      ),
    );
  ...
  Widget _grid(List<List<String>> values) {
